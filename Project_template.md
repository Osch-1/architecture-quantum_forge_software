...
За основу взята вселенная **«Ведьмак» (The Witcher)** — модель знает её очень хорошо, поэтому без переименования отвечала бы «по памяти». Мы построили на её основе **вымышленный мир Аэрдемир / Aerdmir** (тёмное фэнтези о мутантах-охотниках) и **заменили все ключевые термины** — персонажей, локации, чудовищ, магию, фракции, расы, события. После замены тексты остаются логичными и читаемыми, но **не опознаются** как «Ведьмак».

Примеры замен (полный словарь — `terms_map.json`, более 50 соответствий с EN+RU формами):
...
Поскольку источник мы **пишем сами**, корпус написан **сразу в вымышленной лексике** (это гарантирует новизну и грамматически верные русские словоформы). Словарь и инструменты:

- **`terms_map.json`** — словарь соответствий `канон → вымышленное`, сгруппированный по категориям, с полями `canon_en / canon_ru / fic_en / fic_ru`. Общеупотребимые слова (`cat`, `swallow`, `знак`, `континент`…) помечены `"common": true`.
- **`scripts/replace_terms.py`** — переиспользуемый инструмент замены `канон → вымышленное` для любого нового текста (longest-match-first, сохранение регистра, границы слов по Unicode). Демонстрация: *«Geralt … Igni … griffin near Novigrad» → «Korven … Ignar … skyrender near Brennar»*.
- **`scripts/verify_terms.py`** — проверяет, что в `knowledge_base/` **нет ни одного канонического термина** (общеупотребимые `common` пропускаются). Результат: **OK — канонических терминов не найдено в 38 документах.**

### 2.3 Что получилось (результат)

- **`knowledge_base/`** — **38 уникальных документов** (`.md`), один файл = одна сущность, смешанный язык: **19 RU + 19 EN** (`knowledge_base/ru/`, `knowledge_base/en/`).
...
- У каждого документа — frontmatter с метаданными (`id`, `title`, `lang`, `category`, `doc_version`, `created_at`) для цитирования и фильтрации на этапе индексации (Task 3). Документы перекрёстно ссылаются друг на друга — это улучшает retrieval.
- **`terms_map.json`** — словарь замен + пояснение (какую вселенную взяли и как заменили).

**Проверка воспроизводимости:**

```bash
python scripts/verify_terms.py                      # → OK, канонических терминов нет
echo "Geralt cast Igni" | python scripts/replace_terms.py --lang en   # → Korven cast Ignar
```

## Задание 3. Создание векторного индекса

*(в работе)* — `Qwen3-Embedding-0.6B` + FAISS, чанкинг с overlap, метаданные, `build_index.py`.
